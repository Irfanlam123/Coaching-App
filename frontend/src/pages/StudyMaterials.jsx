...
const StudyMaterials = () => {
  ...
  const [materials, setMaterials] = useState([]); // always array
    ...
    const fetchMaterials = async () => {
      try {
        ...
        if (Array.isArray(res.data)) {
          ...
          setMaterials([]); // fallback if response is not an array
        }
      } catch (err) {
  ...
  // Safe classes array
  const classes = ['All Classes', ...new Set(materials?.map(m => `Class ${m.className}`) || [])];
  ...
  return (
    <div className="min-h-screen mt-10 bg-gradient-to-br from-[#043D3B]/5 to-[#043D3B]/10 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="text-center mb-12 relative">
        ...
        {/* Filter Section */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }} className="bg-white rounded-2xl shadow-lg p-6 mb-10 border border-gray-100">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
              <div className="relative flex-1">
                ...
                <input type="text" placeholder="Search materials..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-3 w-full rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#043D3B] focus:border-[#043D3B] transition-all duration-300" />
              </div>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <FaGraduationCap className="h-5 w-5 text-gray-400" />
                </div>
                <select value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)} className="pl-10 pr-4 py-3 w-full md:w-64 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#043D3B] focus:border-[#043D3B] appearance-none transition-all duration-300">
        ...
        {/* Results Count */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.4 }} className="mb-6 flex justify-between items-center">
        ...
        {/* Error Message */}
        {error && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6 text-yellow-700">
            Unable to load materials. Showing default empty list.
          </motion.div>
        )}
            ...
            {filteredMaterials.length > 0 ? (
              <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredMaterials.map((material) => (
                  <motion.div key={material._id} variants={itemVariants} layout className="bg-white rounded-2xl shadow-lg p-6 flex flex-col justify-between border border-gray-100 hover:shadow-xl transition-all duration-300 group">
                    <div>
                      ...
                      <h3 className="text-xl font-semibold text-gray-900 mb-3 group-hover:text-[#043D3B] transition-colors duration-300">{material.materialName}</h3>
                      <div className="space-y-2 mb-4">
                        <p className="text-gray-600 flex items-center"><span className="font-medium mr-2">Subject:</span>{material.subject}</p>
                        <p className="text-gray-500 text-sm flex items-center"><FaClock className="mr-2" />Uploaded: {new Date(material.uploadedAt).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleDownload(material.pdfFile)} className="mt-4 bg-gradient-to-r from-[#043D3B] to-teal-700 text-white py-3 rounded-lg flex items-center justify-center gap-2 hover:shadow-md transition-all duration-300 group-hover:from-teal-700 group-hover:to-[#043D3B]">
